<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#tb').DataTable({
            ajax: "<?php echo base_url(); ?>guruinstansi/ajaxlist",
            scrollx: true,
            responsive: true
        });
    });
    ...
    function add() {
        ...
        $('.modal-title').text('Tambah Guru Instansi');
    }
    ...
    function save() {
        ...
        var quota_siswa = document.getElementById('quota_siswa').value;
        ...
        if (email === '') {
            ...
            alert("Quota siswa tidak boleh kosong");
        } else if (idinstansi === '') {
            ...
            if (save_method === 'add') {
                ...
                url = "<?php echo base_url(); ?>guruinstansi/ajax_add";
            } else {
                url = "<?php echo base_url(); ?>guruinstansi/ajax_edit";
            ...
            form_data.append('quota_siswa', quota_siswa);
    ...
    function hapus(id, nama) {
        if (confirm("Apakah anda yakin menghapus " + nama +
            ...
            $.ajax({
                url: "<?php echo base_url(); ?>guruinstansi/hapus/" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data) {
                    alert(data.status);
    ...
    function ganti(id) {
        ...
        $('#form')[0].reset();
        $('#modal_form').modal('show');
        $('.modal-title').text('Ganti Guru Instansi');
        $.ajax({
            url: "<?php echo base_url(); ?>guruinstansi/ganti/" + id,
            type: "POST",
            dataType: "JSON",
            success: function (data) {
                ...
                $('[name="quota_siswa"]').val(data.quota_siswa);
...
<section class="section">
    <div class="row-btn">
        <ol class="breadcrumb bg-white d-flex align-items-end">
            <h5 class="me-1 mb-0">Daftar Guru Instansi</h5><small>Instansi</small>
        </ol>
    </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form" class="form-horizontal">
                    ...
                    <div class="form-group row">
                        <label for="quota_siswa" class="col-sm-3 control-label">Quota Siswa</label>
                        <div class="col-sm-12">
                            <input type="number" class="form-control" id="quota_siswa" name="quota_siswa"
                                placeholder="Masukan Quota Siswa disini" autocomplete="off">
                        </div>
                    </div>
                    <div class="form-group row">
                            ...
                            <select class="form-control" name="idinstansi" id="idinstansi">
                                ...
                                foreach ($instansi->getResult() as $row) {
                                    ...
                                    <option value="<?php echo $row->idinstansi ?>">
                                        ...
                                        echo $row->nama_instansi . '(' . $row->nama_instansi . ')'; ?>
                                    </option>
                                    <?php
                                }
                                ; ?>
