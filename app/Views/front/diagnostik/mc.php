    ...
    function save() {
        ...
        if(kos > 0){
            ...
            Swal.fire({
                ...
                text:  "There's time left on the timer.",
                icon: "info",
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire("Cancelled", "Keep going with the test!", "error");
                }
            });
        }
    }
                            ...
                            <div class="card-header">
                                <div class="covid-wrap">
                                    <div class="covid-header">
                                        <h2><?php echo $bidang->namabidang; ?> Diagnostic Test</h2>
                                        <input type="hidden" id="idbidang" name="idbidang" value="<?php echo $bidang->namabidang; ?>">
                                        <input type="hidden" id="idpeserta" name="idpeserta" value="<?php echo $kode; ?>">
                                    </div>
                                    <div class="form">
                                        <div class="covid-test-wrap test-step active">
                                            ...
                                            if($row->tipe == "Reading" && $n == 0){
                                                echo '<hr style="border-top: 4px solid purple;">';
                                                $n = 1;
                                            }
                                                if($row->jenis == 'Soal'){?>
                                            <div style="text-align: justify; text-justify: inter-word;" id="divsoal<?php echo $i; ?>">
                                            <?php }
                                            else{
                                                ?>
                                            ...
                                            if($row->audio != ''){ 
                                                $def_audio = base_url().'/images/noimg.jpg';
                                                $audio = $model->getAllQR("select audio from soaltest where idsoal = '".$row->idsoal."';")->audio;
                                                if(strlen($audio) > 0){
                                                    if(file_exists($modul->getPathApp().$audio)){
                                                        $def_audio = base_url().'/uploads/'.$audio;
                                                    }
                                                }?>
                                                <h6 style="color: blue;">Click to start audio! (One-time play) 
                                                <button id="btnaudio_<?php echo $row->idsoal; ?>" name="btnaudio_<?php echo $row->idsoal; ?>" class="btn-lg btn-warning" onclick="playOnce('<?php echo $row->idsoal; ?>')">Play</button></h6>
                                                <audio id="myAudio<?php echo $row->idsoal; ?>" src="<?php echo $def_audio; ?>"></audio>

                                            <?php }
                                            if(str_contains($row->soal, '<p>')){
                                                echo str_replace('p>','h3>',str_replace("<strong>","<b style='color: purple;'>",str_replace("</strong>","</b>",str_replace("<ol","<ol style='color: black;  text-align: left; list-style-type: lower-alpha;'",$row->soal))));
                                            }else{
                                                echo '<h3>'.$row->soal.'</h3><hr>';
                                            }
                                            echo '</div>';
                                            if($row->gambar != null){
                                            ...
                                            <img src="<?php echo $def_foto; ?>" style=" max-width: 100%; height: auto; display: block; margin: 0 auto;"><br>
                                            <?php }
                                            if($row->jenis == 'Soal'){?> 
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-lg-6 col-sm-12">
                                                        <div class="form-group">
                                                            <input type="radio" name="pilihan<?php echo $i;?>" class="form-control" id="<?php echo str_replace('"','',$row1->pilihan).$i;?>" value="<?php echo $row1->idpilihan;?>" required>
                                                            <label for="<?php echo str_replace('"','',$row1->pilihan).$i;?>"><?php echo  htmlspecialchars(str_replace('"','',$row1->pilihan));?></label>
                                                        </div>
                                                    </div>
                                                    <?php } ?>
                                                </div>
                                            ...
                                            <?php } else {
                                                echo '<hr style="border-top: 2px solid white;">';
                                            } 
                                            $i++; }
                                            if($row->jenis =='Instruksi'){
                                                echo '<hr style="border-top: 2px solid grey;">';
                                            }
                                            }
                                            ?>
                                        </div>
                                        <input type="hidden" id="jml" value="<?php echo $jml; ?>">
                                        <button id="btnSimpan" class="btn" onclick="save();">Submit</button>
                                    </div>
                                </div>
                            </div>
                        </div>
